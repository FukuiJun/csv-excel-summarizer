...
UART・ロガーの**どちらか片方だけ**でも Excel にできます。
...
2. ［読み込み →］を押す（どちらか1つ以上選ぶと押せます）。形式が違う場合は原因を表示して画面1にとどまります
...
| グラフ | 1行1グラフ。横軸（初期値 elapsed_ms、経過時間(s) も選べる）、第1軸（左）・第2軸（右）にそれぞれ2つまで項目を載せられる。第1軸の1つ目は必須、それ以外は「なし」も可。同じ項目は1つのグラフで1回まで。横軸が時間（elapsed_ms・経過時間(s)）のときは「横軸の単位」で ms / s / min / h を選べ、元と違う単位を選ぶと解析シートの元の列の右に換算した列（例 time(min)）が追加されて、グラフの横軸になります。［＋ グラフを追加］で追加したグラフは、横軸 elapsed_ms・第1軸にまだどのグラフにも使っていない項目（なければ先頭の項目）が選ばれた状態で追加されます。0個ならグラフシートを作りません |
...
### 片方だけの出力（UART のみ／ロガーのみ）

- 画面1で片方の欄だけ選んで読み込むと、そのデータだけで Excel を作ります（画面2のヘッダーに「（UART のみ）」「（ロガーのみ）」と表示）
- 読み込んでいない方の間隔・オフセット欄は「―」で入力できません
- UART のみ：欠落の補完・二重受信の除去は通常どおり行い、全行を出力します
- ロガーのみ：全行を出力します。経過時間は `(番号 − 1) × ロガー間隔 ＋ オフセット`
- 解析シートは読み込んだ方のブロックだけ、元データのシートもその 1 枚だけになります
- グラフは保存済みの設定から読み込んでいない方の項目を外して作ります（横軸が使えなければ経過時間(s)。描けるものがなければ採用中の項目で 1 つ用意）。
  片方だけの出力では、両方そろったときのグラフ設定を壊さないよう、グラフ設定は config.json に保存しません（項目のラベル・係数・色は保存します）
...
- `assets/LogMerger.ico` は資料の PNG から `python tools/make_ico.py` で作っています（16〜48px は BMP、64px 以上は PNG で格納する標準的な形式）。
  資料の ico は全サイズが PNG 格納で、小さいサイズが使われずにぼやける原因になり得たため作り直しています（絵柄は同じ）
- Windows では起動後に、表示倍率に合ったサイズ（100% なら 32px/16px、150% なら 48px/24px など）を ico から読んでウィンドウに設定します
  （Tk 標準の設定は 32px 前後の 1 枚だけで、タスクバーで拡大・縮小されてぼやけるため）
